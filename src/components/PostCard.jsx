...
function PostCard({
  ...
  const [upCount, setUpCount] = useState(upvotesCount)
  const [downCount, setDownCount] = useState(downvotesCount)


  const upVoteHandler = () => {
    axios({
      method: "PUT",
      url: `${BASE_URL}/post/upvote/${postId}`,
      headers: {
  ...
  const downVoteHandler = () => {
    axios({
      method: "PUT",
      url: `${BASE_URL}/post/downvote/${postId}`,
      headers: {
        accessToken: loggedUser.accessToken,
      },
    }).then((res) => {
    ...
    <div className="post_card">
      <div className="top">
        ...
        <div className="info">
          <p className="username">
            <Link to={`/profile/${username}`}><span>@{username}</span></Link>
        ...
        <div className="menu" onBlur={(e) => viewMenu(false)}>
          <BiDotsVerticalRounded onClick={() => {
            viewMenu(m => !m)
          }} />
          {
            menu ? (
              <motion.div initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0, opacity: 0 }} className="menu-wrapper">
                <div className="tags">
                  <h2>Tags</h2>
                  <ul>
                    <li>#programming</li>
      ...
      <div className="main-part">
        ...
        <div className="actions">
          <button className={`like ${vote === 1 ? "active" : ""}`} onClick={upVoteHandler}>
            <HiOutlineArrowSmUp />
          </button>
          <button className={`dislike ${vote === -1 ? "active" : ""}`} onClick={downVoteHandler}>
      ...
      <div className="states">
        <div className="state likes">
          <HiOutlineArrowSmUp />
          <span className="count">{upCount}</span>
        </div>
        <div className="state dislikes">
          <HiOutlineArrowSmDown />
          <span className="count">{downCount}</span>
        </div>
